...
Tree* FindNodeInBST(Tree* t, int k)
{	
    if (t == NULL) return NULL;
    if (k==t->key)
        return t;
   ...
   if (k < t->key)
        return FindNodeInBST(t->left, k);
        
    if (k > t->key)
        return FindNodeInBST(t->right, k);
...
int main()
{
    ...
    do {
        ElType a;
        printf("Podaj klucz elementu do znalezienia w liscie: ");
        scanf("%d", &a);
	...
	if (FindNodeInBST(t, a)==NULL) {
        	printf("Klucz elementu: %d nie zostal znaleziony", a);
	} else {
		printf("Klucz elementu: %d zostal znaleziony", a);
	}
        DisplayTree(t);
    } while (1);
